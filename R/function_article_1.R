...
moving_window_categories_3 <- function(data,
  ...
  data_clean$first_window <- NA
  data_clean$last_window <- NA
  
  for (i in seq_len(n_windows)) {
    idx <- start_indices[i]:(start_indices[i] + size - 1)
    for (j in idx) {
      if (is.na(data_clean$first_window[j])) data_clean$first_window[j] <- i
      data_clean$last_window[j] <- i
    }
    subset <- data_clean[idx, ]
    classes <- subset$categorie
    tab <- table(factor(classes, levels = ordered_subtypes))
    prop_matrix[, i] <- tab / sum(tab)
    median_ord[i] <- median(subset[[ordination]], na.rm = TRUE)
  }
  
  # Agrégation des doublons de médiane
  ...
  colnames(y_mat) <- ordered_subtypes
