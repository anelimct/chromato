merge_datasets <- function(df_ERfield_mono, df_ERfield_iso, bvocs_samples){
  ...
  bvocs_samples_ER <- merge(bvocs_samples, df_iso_mono, by = "ID") |> dplyr::filter(!is.na(Leaves_DM)) |>   dplyr::mutate(across(all_of(cols_PAR), as.numeric)) |>  dplyr::mutate(PAR_algo = rowMeans(dplyr::across( all_of(cols_PAR)), na.rm = TRUE)) |> 
    ...
    dplyr::select(ID, monoterpenes, isoprene, Taxon, PAR_algo, T_algo_K ) |>  tidyr::pivot_longer(cols = c(isoprene, monoterpenes),names_to = "Compound",values_to = "Emission") |> dplyr::mutate(Standardized = FALSE) |>  apply_standardization() |>  dplyr::mutate(EF = ES_iso_G93) |>  dplyr::select(EF, Taxon, Compound) |> dplyr::mutate(source = "field") |>  dplyr::mutate(Taxon = stringr::str_replace_all(Taxon, " ", "_"))
