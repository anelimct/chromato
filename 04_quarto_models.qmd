...
imputed.traits_3T <- subset(imputed.traits_3T, select = -c(SeedMass, DispMode))
imputed.traits <-  imputed.traits_3T
funspace::funspaceDim(imputed.traits) #3 dimensions
imputed.traitsN <- normaliser_dataframe(imputed.traits)
pca.trait <- princomp(imputed.traitsN, cor=TRUE)
summary(pca.trait)
trait_space_global <- funspace::funspace(pca.trait, PCs=c(2,3), group.vec = merged_data_3T$BVOCsData)
...
isoprene <- merged_data_3T$isoprene[!is.na(merged_data_3T$isoprene)]
names(isoprene) <- row.names(dplyr::filter(merged_data_3T, !is.na(isoprene)))


create_residual_correlogram(tree, isoprene)
```
...
rownames(df) <- rownames(merged_data_3T)
  ...
  # Garder uniquement les tips avec des valeurs non-NA pour le trait "Sum"
tips_to_keep <- rownames(tdata(phylo4d_object, type = "tip"))[!is.na(tdata(phylo4d_object, type = "tip")$Sum)]

phylo4d_object_clean <- subset(phylo4d_object, tips.include = tips_to_keep)
                               
                               
 clust <-  phylosignal::graphClust(phylo4d_object_clean,trait = "Sum")
  print(clust)
...
hist(log(merged_data_3T$Sum+0.01))
...
modele_s <-nlme:: gls(log(Sum+0.01) ~ Comp.2+ Comp.3 + Comp.1 +Comp.2*Comp.3 , data = merged_data_3T, method = "REML", na.action = na.omit)
summary(modele_s)
...
Soit le model est surparamétré par rapport au nombre d'observations, ou un manque d'observation de certaines intéractions ? **A revoir,** en fait il y avait surtout des 0, enfin 1 UGLA et il ne de vait pas y en avaoir avant, j'ai mis log +1 maintenat il n'y a aplus d'erreur mais il faudrait quand meme revoir ce model, les résidus ne sont plus du tout normaux.
...
model_glm_phy <- phyr::pglmm_compare( log(Sum+0.01) ~Comp.2+ Comp.3 + Comp.1 +Comp.2*Comp.3 , family = "gaussian", phy = tree , data = merged_data_3T, REML = T, na.action(na.omit))
...
```{r}
plm <-phylolm::phylolm(formula = log(Sum+0.01)  ~ Comp.2+ Comp.3 + Comp.1 +Comp.2*Comp.3  , data = merged_data_3T, REML = TRUE, phy =tree, model = "BM")
summary(plm)
hist(plm$residuals)
...
plm_boot <-phylolm(formula = log(Sum+1)  ~ Comp.2+ Comp.3 + Comp.1 +Comp.2*Comp.3  , data = merged_data_3T, REML = TRUE, phy =tree, boot = 250)
...
model_glm_phy <- pglmm_compare( log(Sum+0.01) ~ Comp.2+ Comp.3 , family = "gaussian", phy = tree , data = merged_data_3T, REML = T)
summary(model_glm_phy)
rr2::R2(model_glm_phy)
hist(model_glm_phy$H)
shapiro.test(model_glm_phy$H)
```
