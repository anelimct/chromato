...
```{r}
# traits <- INTEGRADIV_traits_v5 |> dplyr::filter(Taxon == "Trees")|> dplyr::select(Species, Trait, Value) |> pivot_wider(names_from = Trait, values_from = Value)|>   dplyr::mutate(Species = stringr::str_replace(Species, " ", "_")) |> tibble::column_to_rownames(var = "Species")|> dplyr::select( DispMode, HeightMax, LeafArea, SLA, SeedMass, StemSpecDens) |> mutate(DispMode = as.factor(DispMode)) |> mutate(across(-1, as.numeric))
```

```{r}
# targets::tar_source(here::here("R/analysis_internship"))
...
#in the trait database there is no imputed values for liquidambar orientalis SLA ans StemSpecDens

data_traits_EF <-merge_trait_EF(imputed.traits_3T, DB_bvocs_iso_mono_EF)
data_traits_EF <- subset(data_traits_EF, select = -c(SeedMass, DispMode, Sum, total, p_isoprene, p_monoterpenes, prct_isoprene, prct_monoterpenes, isoprene_mod, binaire  )) 


data_traits_all_sp <- data_traits_EF |>  subset( select = -c(isoprene, monoterpenes, BVOCsData, type)) |> normaliser_dataframe() #ajouter en groupe vec BVOCsData to create a subset for which we have isoprenoids data 


#vecteur group

type_isoprenoids <- data_traits_EF|> dplyr::filter(BVOCsData == 1) |>subset( select =  type)

funspace::funspaceDim(data_traits_all_sp) ## retains only the first compoenent ? 
pca.trait_usual_all <- princomp(data_traits_all_sp, cor=TRUE)
summary(pca.trait_usual_all)




pca.trait <- pca.trait_usual_all
trait_space_global <- funspace(pca.trait, PCs=c(2,3), group.vec = merged_data_3T$BVOCsData)
...
TPD <- trait_space_global$global$images$noThreshold.quantiles |>
  reshape2::melt()|> 
  dplyr::rename(PC2 = Var1) |> 
  dplyr::rename(PC3 = Var2) 
```
